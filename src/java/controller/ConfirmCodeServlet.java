...
//        String cap = request.getParameter("cap").trim();
        ...
        String enterCode = request.getParameter("verify");
        String verify = (String) session.getAttribute("vericode");
      

        String mess = "";
        if (enterCode.length() == 0 || ! enterCode.equals(verify) ) {
            mess = "You have to input your exactly code ";
            request.setAttribute("mess", mess);
            request.getRequestDispatcher("ConfirmCode.jsp").forward(request, response);
            return;
        } else {
            mess = "Enter captcha successfully";
            request.setAttribute("mess", mess);
